...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Бег":
		distance := distance(action)                               // считаем дистанцию
		speed := meanSpeed(action, duration)                       // вычисляем скорость
		calories := RunningSpentCalories(action, weight, duration) // расчитываем расход калорий при беге
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	case trainingType == "Ходьба":
		distance := distance(action)                                       // считаем дистанцию
		speed := meanSpeed(action, duration)                               // вычисляем скорость
		calories := WalkingSpentCalories(action, duration, weight, height) // расчитываем расход калорий при ходьбе
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	case trainingType == "Плавание":
		distance := distance(action)                                               // считаем дистанцию
		speed := swimmingMeanSpeed(lengthPool, countPool, duration)                // вычисляем скорость
		calories := SwimmingSpentCalories(lengthPool, countPool, duration, weight) // расчитываем расход калорий при плаваньи
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	default:
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	heightImM := height / 100
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(walkSpeedInMsec, 2)/heightImM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь (СредняяСкоростьВКм/ч + 1.1) * 2 * ВесСпортсменаВКг * ВремяТренеровкиВЧасах
